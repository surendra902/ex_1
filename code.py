...
class Config:
    ...
    # When true, skip directional evaluate_single fan-out (LatArb-only mode).
    ...
    def validate(self) -> List[str]:
        ...
        if not self.dry_run and (not self.require_proven_edge):
            errs.append('REQUIRE_PROVEN_EDGE=false is not allowed while DRY_RUN=false; directional live trading requires proven calibrated edge')
...
def evaluate_go_no_go(cfg: 'Config') -> Tuple[bool, List[str]]:
    ...
    return go_no_go(report, min_samples=cfg.min_proven_samples, min_edge=cfg.min_proven_edge, max_adverse_bps=cfg.max_adverse_bps)
...
class LatencyArb:
    ...
    def __init__(self, cfg: Config, om: OrderManager, tracker: PriceTracker, polyfeed: HyperPolyFeed, by_coin: Dict[str, List[Market]]):
        ...
        self._live_proof_checked_at: float = 0.0
    ...
    def _refresh_live_proof(self) -> bool:
        now = time.monotonic()
        if now - self._live_proof_checked_at < 60.0:
            return self._live_proof_ok
        self._live_proof_checked_at = now
        if self.cfg.dry_run or not getattr(self.cfg, 'require_latarb_live_proof', False):
            # Without the gate, keep min-size only (scale only after live-proof GO).
            self._live_proof_ok = False
            return False
        try:
            ok, _, stats = evaluate_latarb_live_proof(self.cfg)
            mode = (stats or {}).get('mode', 'pass')
            self._live_proof_ok = bool(ok) and mode != 'bootstrap'
        except Exception:
            self._live_proof_ok = False
        return self._live_proof_ok
    ...
    async def _eval_market(self, mkt: Market, coin: str, price: float, _bankroll: float, oracle_px: Optional[float]=None) -> None:
        ...
        # Persist open for settle meta even when LATARB_ONLY skips directional evaluate.
...
class Bot:
    ...
    async def _boot(self) -> None:
        ...
        measure_only = False
        if self.cfg.require_proven_edge:
            ok_go, reasons = evaluate_go_no_go(self.cfg)
            if not ok_go:
                if self.cfg.dry_run:
                    measure_only = True
                    self.log.warning('GO/NO-GO: MEASURE-ONLY (dry-run) â€” no proven edge; calibration evals/outcomes will be logged but NO orders will be placed.  Reasons:')
                    for rsn in reasons:
                        self.log.warning('  - %s', rsn)
                    self.log.warning('  Inspect with `python polybot.py --analyze %s`.  Set REQUIRE_PROVEN_EDGE=false only if you accept trading an unvalidated model.', self.cfg.calibration_log_path)
                else:
                    self.log.critical('GO/NO-GO: NO-GO â€” refusing to trade DIRECTIONAL (live) with real capital until measured edge > measured cost:')
                    for rsn in reasons:
                        self.log.critical('  - %s', rsn)
                    if self.cfg.latency_arb_enabled:
                        self.log.critical('  DIRECTIONAL -> MEASURE-ONLY.  LATENCY_ARB_ENABLED=true, so latency-arb WILL trade live â€” confirm its edge first with `--latarb-analyze`.')
                        measure_only = True
                    else:
                        msg = 'GO/NO-GO: NO LIVE STRATEGY ENABLED. Run DRY_RUN to collect data, then `python polybot.py --analyze %s`.' % self.cfg.calibration_log_path
                        self.log.critical('  %s', msg)
                        raise FatalBotError(msg)
            else:
                self.log.info('GO/NO-GO: GO â€” %s', '; '.join(reasons) if reasons else 'gate cleared')
        ...
        if _forced_measure_only:
            if not measure_only:
                self.log.info('MEASURE-ONLY (forced via --measure-only): logging evals/outcomes, placing NO orders regardless of the go/no-go verdict.')
            measure_only = True
        if self.cfg.latency_arb_enabled and (not self.cfg.dry_run) and self.cfg.require_latarb_proven_edge:
            ...
            if not ok_lat:
                ...
                canary_ok = canary_ok and (not bool(getattr(self.cfg, 'directional_enabled', True)))
        ...
        self.fivemin.measure_only = measure_only
    ...
    async def _on_book(self, tid: str, book: OrderBook) -> None:
        ...
        if not self.risk.ok():
            return
        if self.cfg.event_driven and self.fivemin and m.coin and getattr(self.cfg, 'directional_enabled', True):
            now = time.monotonic()
            ...
            try:
                t = asyncio.create_task(self.fivemin.evaluate_single(m, self.fivemin_markets), name=f'eval_{m.market_id[:8]}')
    ...
    async def _on_price(self, coin: str, price: float) -> None:
        ...
        if self.cfg.event_driven and self.fivemin and getattr(self.cfg, 'directional_enabled', True):
            now = time.monotonic()
            markets = self.by_coin.get(coin, [])
            for mkt in markets:
                ...
                try:
                    t = asyncio.create_task(self.fivemin.evaluate_single(mkt, self.fivemin_markets), name=f'eval_{mkt.market_id[:8]}')
                    self._eval_tasks.add(t)
                    t.add_done_callback(self._eval_tasks.discard)
                except Exception as e:
    ...
    def _banner(self) -> None:
        ...
        self.log.info('  LatArb   : enabled=%s  shadow=%s  directional=%s  age=%.0f..%.0fms  edge=%.3f  min_prob=%.2f  proof=%s/%s', self.cfg.latency_arb_enabled, self.cfg.latarb_shadow, self.cfg.directional_enabled, self.cfg.latarb_shadow_min_age_ms, self.cfg.latarb_shadow_max_age_ms, self.cfg.latency_arb_edge, self.cfg.latency_arb_min_prob, self.cfg.require_latarb_proven_edge, self.cfg.require_latarb_live_proof)
...
def main() -> None:
    ...
    parser.add_argument('--measure-only', action='store_true', help='Online: connect to all feeds, log every EVAL snapshot and OUTCOME to the calibration CSV, but place NO orders.  Forces the strategy into MEASURE-ONLY mode (every entry is a no-op) regardless of require_proven_edge.  Safe way to accumulate the data needed to clear the go/no-go gate without risking capital.')
